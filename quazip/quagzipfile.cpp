                              ...
                              QString &error)
{
    ...
    if ((mode & QIODevice::Append) != 0) {
        error = QString::fromUtf8("QIODevice::Append is not "
                "supported for GZIP");
        return false;
    }
    if ((mode & QIODevice::ReadOnly) != 0
            && (mode & QIODevice::WriteOnly) != 0) {
        error = QString::fromUtf8("Opening gzip for both reading"
        ...
        error = QString::fromUtf8("You can open a gzip either for reading"
            " or for writing. Which is it?");
        return false;
    }
    gzd = open(id, modeString);
    if (gzd == NULL) {
        error = QString::fromUtf8("Could not gzopen() file");
        return false;
    }
    return true;
}
