...
public:
    ...
    Q_INVOKABLE void createShortCut(QString keys, QJSValue scriptFunction)
    {
        ...
        {
            m_scriptThread->getScriptEngine()->throwError("could not find function: " + scriptFunction.toString());

        }
#endif
        connect(shortCut, &shortCut->activated, [scriptFunction](){scriptFunction.call();});
    }
