...
struct ScriptTableCellPosition
{
    ...
    static QJSValue toScriptValue(QJSEngine *engine, const ScriptTableCellPosition &s)
    {
    ...
    static void fromScriptValue(const QJSValue &obj, ScriptTableCellPosition &s)
    {
      s.row = obj.property("row").toInt();
      s.column = obj.property("column").toInt();
    }

    static QJSValue createScriptTableCellPosition(QJSEngine *engine)
    {
        ScriptTableCellPosition s;
        return engine->toScriptValue(s);
    }


    // static void registerType(QJSEngine* engine)
    // {
    ...
    //     // QJSValue ctor = engine->newFunction(createScriptTableCellPosition);
    //     // engine->globalObject().setProperty("ScriptTableCellPosition", ctor);
    //     QJSValue ctor = engine->newQObject(new ScriptTableCellPosition());
    //     engine->globalObject().setProperty("ScriptTableCellPosition",  ctor.property("createScriptTableCellPosition"));
    // }
};
        ...
        for(int i = 0; i < ranges.length(); i++)
        {
            ...
            for(int column = currentRange.leftColumn(); column <= currentRange.rightColumn();column++)
            {
                ...
                for(int row = currentRange.topRow(); row <= currentRange.bottomRow();row++)
                {
                    ...
                    pos.column = column;
                    pos.row = row;
                    result.append(ScriptTableCellPosition::toScriptValue(m_scriptThread->getScriptEngine(), pos));
