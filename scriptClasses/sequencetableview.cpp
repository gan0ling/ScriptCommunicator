 ...
 *      The drag move event.
 */
void SequenceTablePlainTextEdit::dragMoveEvent(QDragMoveEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        event->acceptProposedAction();
    ...
    if(!sendScript->isEmpty() && !debugWindowHasBeenClosed)
    {
        ...
        if (*scriptEngineWrapper)
        {
            ...
            if (!(*scriptEngineWrapper)->sendDataFunction->isError())
            {
                ...
                {
                    // scriptArray.setProperty(i, QJSValue((*scriptEngineWrapper)->scriptEngine, (unsigned char)(sendData->at(i))));
                    scriptArray.setProperty(i, (*scriptEngineWrapper)->scriptEngine->toScriptValue(sendData->at(i)));
                }
            ...
            if((*scriptEngineWrapper)->scriptEngine->hasError())
            {
                QJSValue exception = (*scriptEngineWrapper)->scriptEngine->catchError();
