...
public:
    explicit ScriptSound(QObject *parent, QString filename) : QObject(parent) 
    {
        ...
        m_sound.setSource(QUrl::fromLocalFile(filename));
    }
    ...
    Q_INVOKABLE QString fileName(void){return m_sound.source().toString();}
    ...
    Q_INVOKABLE int loops(void){return m_sound.loopCount();}
